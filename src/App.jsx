...
function buildCvpBasicSteps(c) {
  ...
  s.push({
    ...
    qs: [
      ...
      { step: '4／4　手法の使い分け',
        q: '別の年度の設問で「販売費は固定費、材料費は変動費とする」と指示があった。どう分解するか。',
        opts: [
          { t: '勘定科目精査法。指示どおり科目ごとに集計し、高低点法は使わない', ok: true },
          { t: '高低点法で求めた変動費率と照らし合わせて平均をとる' },
          { t: '高低点法を優先する。より客観的な方法だから' },
          { t: '指示された科目を除いたうえで高低点法をあてはめる' }],
        why: '固変分解には高低点法と勘定科目精査法があり、どちらを使うかは設問文が決める。「変動費率は一定とする」なら高低点法、「何々費は固定費」と科目ごとの指示があれば勘定科目精査法。ここを取り違えると以降の設問を連鎖して外すので、解き始める前に必ず設問文を確認する。なお事例IVの高低点法は、生産量ではなく売上高の差で費用の差を割る形で問われることが多い。' }],
  });
...
function CvpDrill({ onFinish, onExit }) {
  return (
    <DrillRunner
      ...
      modes={[
        { id: 'basic', icon: '📐', title: '基本', desc: '固変分解 → 変動損益計算書 → BEP売上高・安全余裕率・営業レバレッジ。9項目15問。', xp: 90 },
...
const SIGNAL_CARDS = [
  ...
  // ===== 解答の型（全事例共通） =====
  { case: 'common', q: '設問で「課題」と問われた。「問題点」との書き分けは？',
    a: '問題点は与件にあるネガティブな事実そのまま。課題はあるべき姿に向けて取り組むべきことを前向きに書く。「進んでいない」ではなく「進めること」。' },
  { case: 'common', q: '140字クラスの長い助言問題。答案の組み立て方は？',
    a: '結論プラス方法並列型。結論を先に一文で置き、そのあと方法を二つ並べる。方法どうしの字数は均等にする。' },
  { case: 'common', q: '設問文に「統合前の」「この業界における」のような限定がついている。どう扱うか？',
    a: '制約条件。外すと題意ズレで大きく失点する。書き始める前に設問文の制約に印をつけ、要素がすべてその範囲に収まっているか確かめる。' },
  { case: 'common', q: '第1問で強みと弱みを問われた。どの順で解くか？',
    a: '最終問の戦略を先に決め、その戦略で活かす強みと、克服する弱みを選ぶ。第1問を単独で解くと後半の設問とつながらない。' },
  { case: 'common', q: '与件文に接続詞の「また」があった。何を意味するか？',
    a: '前後が並列。出題者が整理の切り口として置いていることが多い。「また」の前後で二つに分けると、そのまま解答の二要素になる。' },
  { case: 'common', q: '与件文に、どの設問にも関係なさそうな情報が書かれている。どうするか？',
    a: 'わざわざ書かれた情報は必ずどこかの設問で使う。使っていない段落が残ったら、設問の読み違いを疑う。' },
  { case: 'common', q: '施策を助言する。効果まで書けとは明示されていない。どうするか？',
    a: '効果まで書く。何々により何々し、何々を高める、という形で因果をつなぐ。施策の羅列だけでは論理の飛躍と見なされる。' },
  { case: 'common', q: '解答要素を二つ書く。字数配分で気をつけることは？',
    a: '均等に配分する。片方が極端に短いと、その要素は薄いと判断される。二つなら半分ずつを目安にする。' },
  { case: 'common', q: '与件文の記述と一次知識、どちらを優先して書くか？',
    a: '与件が優先。知識は与件の事実を説明する言葉として添える。与件に根拠のない知識だけの解答は点が入らない。' },

  // ===== 事例I 追加 =====
  { case: 'case1', q: '組織が一体となって機能していない。組織が成り立つ三つの要件は？',
    a: '共通目的、貢献意欲、コミュニケーション。バーナードの組織成立三要件。どれが欠けているかを与件から特定し、そこを埋める助言をする。' },
  { case: 'case1', q: '今後の経営戦略を助言せよ、と問われた。どの切り口で考えるか？',
    a: '強みと機会を掛け合わせる。誰に何を売るかは市場と製品の組み合わせで整理する。既存市場か新市場か、既存製品か新製品か。' },
  { case: 'case1', q: '先代経営者と現経営者のやり方が対比して書かれている。どう使うか？',
    a: '何を変えたかが論点。ターゲット、商品、価格、チャネルの四つで並べて比べると、変化の中身がそのまま解答になる。' },
  { case: 'case1', q: '企業買収や経営統合のあとの組織をどうするか、と問われた。切り口は？',
    a: 'シナジー。両社の強みを組み合わせて相乗効果を出す。あわせて組織文化の統合を図り、方向性の共有と相互の助け合いで社員の不安を解消する。' },

  // ===== 事例II 追加 =====
  { case: 'case2', q: '第1問で3C分析を問われた。どの順で何を見るか？',
    a: '顧客、競合、自社の順。顧客は需要がどう増減したかという変化、競合は誰が競合でどんな特徴か、自社はその競合と差別化できる強み。' },
  { case: 'case2', q: '新規顧客の獲得を助言する。最初に特定すべきものは？',
    a: '接点。狙う客とどこで出会えるか。与件に書かれた立地や施設の情報が接点の候補になる。接点が決まれば打ち手は自然に決まる。' },
  { case: 'case2', q: '顧客の金銭的負担を減らす販売方法を問われた。新しい価格の付け方は？',
    a: 'サブスクリプションとダイナミックプライシング。くり返し使うものは定額にし、需要に波があるものは時期によって価格を変える。' },
  { case: 'case2', q: '施策を助言する。書き落としやすい要素は？',
    a: '誰に。ターゲットを絞らずに施策だけ書くと点が伸びない。誰に、何を、どのように、そして効果、の順で組み立てる。' },

  // ===== 事例III 追加 =====
  { case: 'case3', q: '生産面の課題を問われた。まず何で二分するか？',
    a: '生産計画と生産統制。計画は何をいつどれだけ作るか、統制は計画どおり進んでいるかの管理。どちらの話かを決めてから中身を考える。' },
  { case: 'case3', q: '生産計画の話だと決まった。日程計画は何段階に分かれるか？',
    a: '大日程、中日程、小日程。大日程は三か月から一年で資材の手配、中日程は一か月から三か月で作る種類と数、小日程は一週間ほどで作業の割り当て。' },
  { case: 'case3', q: 'ある工程だけ負荷が大きく、いつも遅れている。どう手を打つか？',
    a: '余力管理。生産能力と負荷の差を見て、その工程の作業を他工程へ移すか、作業の割り当て基準を熟練度ではなく生産能力に変える。' },
  { case: 'case3', q: '生産計画が月一回で、月の途中に何度も変更されている。どうするか？',
    a: '立案の頻度を上げる。月一回を週一回にして受注の変動や特急品を吸収する。あわせて大日程計画で資材を先に手配し、欠品による変更をなくす。' },
  { case: 'case3', q: '設問が複数あり、どこまで書くと重複するか分からない。どう切り分けるか？',
    a: '製品や工程の単位で棲み分ける。受託品の話か自社ブランドの話か、成長戦略の話か生産現場の話か。テーマの違いを決めてから書き始める。' },

  // ===== 事例IV =====
  { case: 'case4', q: '経営分析で指標を選んだ。原因を60字から80字で書く。型は？',
    a: '主語、原因、結果、知識の四つ。何々率悪化の原因は、与件の事情により、売上は減ったが費用は減らず、収益性が下がっているため。' },
  { case: 'case4', q: '設問に「変動費率は一定とする」とある。固変分解はどの方法か？',
    a: '高低点法。二期の費用の差を売上高の差で割って変動費率を出す。事例IVでは生産量ではなく売上高で割ることが多い。' },
  { case: 'case4', q: '設問に「販売費は固定費、材料費は変動費とする」とある。どう分けるか？',
    a: '勘定科目精査法。指示どおり科目ごとに足し引きするだけで、高低点法は使わない。設問文のどちらの指示かで手法が決まる。' },
  { case: 'case4', q: '製品の製造中止を判断する。どの利益で決めるか？',
    a: '貢献利益。限界利益から個別固定費を引いた額で、これが正なら中止しない。共通固定費は中止しても残るので判断に入れない。' },
  { case: 'case4', q: '新規の投資や増産をするか判断する。どの利益で決めるか？',
    a: '限界利益。売上から変動費を引いた分がいくら増えるかを見る。中止の判断は貢献利益、投資の判断は限界利益、と使い分ける。' },
  { case: 'case4', q: '共通費を売上高だけを基準に各製品へ配賦している。何が問題か？',
    a: '単一の基準による配賦は経営実態とずれる。活動基準原価計算を使い、活動ごとに配賦して製品別のより正確な原価をつかむ。' },
  { case: 'case4', q: '投資の実行を一年遅らせる案がある。どう計算するか？',
    a: '一年後を起点にNPVを出し、最後に一年分の現価係数を掛けて現在に割り戻す。遅らせる間に情報が得られ、投資しない選択ができる点が価値になる。' },
  { case: 'case4', q: '販売量が二通りあり、それぞれ確率が与えられている。NPVはどう出すか？',
    a: '各シナリオのNPVを出してから確率で加重平均する。投資しない選択ができる設定なら、マイナスのシナリオはゼロとして計算する。' },
  { case: 'case4', q: '設問に「小数点第三位を四捨五入」とある。気をつけることは？',
    a: '途中では丸めず、指示された箇所だけ丸める。前の設問で求めた値を使えとあればその丸めた値を使う。端数処理は配点に直結する。' },
  { case: 'case4', q: '簿価のある設備を処分する。キャッシュフローのどこに何を書くか？',
    a: '処分損益は営業CFで打ち消し、処分による収入は投資CFに実額で計上する。上で消して下で実額、が原則。' },
  { case: 'case4', q: 'OEM生産を委託する側の財務的な利点は？',
    a: '生産設備への投資が不要なので効率性が高く、借入の負担も小さいので安全性も高い。自社の強みに資源を集中できる。' },
  { case: 'case4', q: 'OEM生産をやめて自社生産に切り替える財務的な利点は？',
    a: '高付加価値品を自社で開発し販売できるうえ、既存の従業員を活用できるので収益性を確保できる。反面、設備投資と借入は増える。' },

  // --- 体系（事例I）---
  { case: 'case1', q: '中小企業のA社が、大手と同じ土俵で戦わない。何の経済性を追うか？',
    a: 'スピードの経済性とネットワークの経済性。市場や顧客の変化に先んじる、あるいは変化と同期化する。規模や範囲の経済性は大手の戦略。' },
  { case: 'case1', q: '経営資源のシナジーを考える。中小企業はどこを重視するか？',
    a: '無形資源と有形資源の組み合わせ。量ではなく質。技術やノウハウ、ブランドといった無形資源を軸に据える。' },
  { case: 'case1', q: '与件に書かれたマクロ環境の変化は、大きく四つに分けられる。何か？',
    a: '複雑化する市場、激化する競争、進化する技術、ルールの変更。市場の成熟とニーズの二極分化、業際化、技術革新、法規制の変化。' },
  { case: 'case1', q: '戦略と組織の関係を問われた。押さえる二つの言葉は？',
    a: '組織は戦略に従う、戦略は組織に従う。組織構造は新しい経営戦略に合わせて変え、その経営戦略は創業からの経営理念に従う。' },
  { case: 'case1', q: '組織構造の変革を助言する。よりどころにする管理原則は？',
    a: 'まず専門化の原則。そのうえで、権限と責任の一致の原則か、例外の原則つまり権限委譲の、どちらかを選ぶ。' },
  { case: 'case1', q: '組織文化のマイナス面を指摘する。何を挙げるか？',
    a: '経営戦略との不整合、機会主義つまり自己利益の追求、組織的なコンフリクト。プラス面は与件に寄り添って気づかせる。' },
  { case: 'case1', q: '事業部制組織のデメリットは？',
    a: '組織スラック。事業部ごとに資源を持つので重複と余剰が生まれる。全社最適が失われ部分最適に陥る点もあわせて指摘する。' },
  { case: 'case1', q: 'マトリックス組織の最大の問題は？',
    a: 'ツーボスシステム。指示命令が二系統になり、責任と権限があいまいになる。機能別と事業部制の利点を両取りする代償。' },
  { case: 'case1', q: '人事施策を助言する。押さえる二つの目的は？',
    a: '能力向上とモラール向上。どちらを狙うのかを設問と与件から読み取り、対象となる従業員を特定してから手段を挙げる。' },
  { case: 'case1', q: '能力向上が目的と決まった。専門知識と対応力、どちらを伸ばすか？',
    a: 'その従業員が持っていないほう。研究職のように専門知識が高い相手には対応力を、接客のように対応力が高い相手には専門知識を。' },
  { case: 'case1', q: '能力を高める手段は三つある。試験で使ってよいのはどれか？',
    a: '職場でのOJTと、社外でのOFF-JT。自己啓発は設問や与件に書かれていない限り使わない。' },
  { case: 'case1', q: 'モラール向上が目的。動機づけ要因には何があるか？',
    a: '職務充実、職務拡大、達成、承認、責任、昇進。仕事そのものの中身と、認められることが満足につながる。' },
  { case: 'case1', q: '衛生要因には何があるか？',
    a: '会社の方針、指示命令、給与、作業条件、労働時間、人間関係。足りないと不満になるが、満たしても満足までは生まない。' },

  // --- 体系（事例II）---
  { case: 'case2', q: '商品やプロモーションを考える。顧客が持つ二つの価値とは？',
    a: '機能的価値と情緒的価値。機能で引きつけて情緒で生活に浸透させるか、情緒で引きつけて機能で納得して買ってもらうか。' },
  { case: 'case2', q: '小さな店が大きな店を超えるための三つの絞り込みは？',
    a: 'ターゲットを絞る、商品を絞る、知恵を絞る。誰に売るかを決め、本物力の出る商品に集中し、小規模ならではの工夫で付加価値をつける。' },
  { case: 'case2', q: '市場細分化の変数は四つに分けられる。何か？',
    a: '量的側面が地理的変数と人口動態変数、質的側面が心理的変数と行動変数。与件の文言をこの四つに仕分けてからターゲットを選ぶ。' },
  { case: 'case2', q: '行動変数と心理的変数の違いは？',
    a: '行動変数は購買の頻度や経路など、実際に買った事実で分ける。心理的変数は価値観や趣向、ライフスタイルという感性で分ける。' },
  { case: 'case2', q: 'ポジショニングを分析する。最初にやることは？',
    a: '競合の特定。与件の競合情報が商品の話かサービスの話かを見極め、それを対比軸にして自社が反対の位置を取る。' },
  { case: 'case2', q: '競合が低価格で総合的な品揃えだった。自社はどこを取るか？',
    a: '高品質で専門的な品揃え。価格では戦わず、質と専門性で選ばれる位置に立つ。与件の言葉に置き換えて表現する。' },
  { case: 'case2', q: '3C分析とSWOT分析はどう対応するか？',
    a: '顧客は機会、競合は脅威、自社は強みと弱み。どちらで問われても答えられるよう、与件整理の段階で両方に紐づけておく。' },
  { case: 'case2', q: '小規模事業者ならではの強みは何か？',
    a: '個性化力、変化対応力、対面販売力、地域密着力。大手にできない小回りと顔の見える関係が、そのまま差別化の源になる。' },
  { case: 'case2', q: '継続的な接触活動で、何を高めるのか？',
    a: '顧客生涯価値。買ったあとも接点を持ち続けて再購買につなげる。接触のタイミングを商品特性に合わせるのがきずな力。' },
  { case: 'case2', q: '顧客満足を高める段階と、地域の幸せまで踏み込む段階を何と呼び分けるか？',
    a: '顧客とのきずなはマーケティング2.0、地域とのきずなはマーケティング3.0。3.0は社会価値と企業価値の両立をねらう。' },
  { case: 'case2', q: '商品を絞ったあと、売上を伸ばすにはどうするか？',
    a: 'アイテムを充実させ、顧客の生活起点で品揃えの幅を広げる。商品の種類でくくる業種店から、暮らしの場面でくくる業態店へ移る。' },

  // --- 体系（事例III）---
  { case: 'case3', q: '事例IIIの設問は大きく二層に分かれる。何と何か？',
    a: '成長戦略のレベルと生産戦略のレベル。生産の事例だが配点の半分は成長戦略なので、どちらを問われているかを先に見極める。' },
  { case: 'case3', q: '成長ベクトルの二つとは？',
    a: '新市場開拓戦略と新製品開発戦略。既存製品を新しい販路へ出すか、既存顧客へ新しい製品を出すか。与件の取引先と製品を整理して選ぶ。' },
  { case: 'case3', q: '成長ベクトルを一文で書く。どの三要素で表すか？',
    a: '誰に、何を、独自能力。ドメインの三要素。機会と強みを掛け合わせた結果として、新市場開拓か新製品開発かを示す。' },
  { case: 'case3', q: '生産戦略の枠組みは二つの軸でできている。何と何か？',
    a: '品質、原価、納期の軸と、設計、調達、作業の軸。この組み合わせで、どの範囲を問われているかを絞り込む。' },
  { case: 'case3', q: '設問に「生産管理」という言葉があった。どの切り口で解くか？',
    a: '作業と納期が交わるところ。生産計画と生産統制の二つに分けて問題解決する。' },
  { case: 'case3', q: '生産管理の設問と、生産性向上の設問はどう書き分けるか？',
    a: '生産管理は人に頼らない仕組みを作る話。生産性向上は人手不足に対して人の生産性を高める話で、多能工化と兼任化が方向性になる。' },
  { case: 'case3', q: '情報技術の活用を助言する。どう発想するか？',
    a: '離れている二つの機能を双方向でつなぐ。片方だけが持っている情報をもう片方と共有し、相乗効果を出す。' },
  { case: 'case3', q: '情報技術の設問を書くときの注意は？',
    a: '手段なので目的を確認する。他の生産戦略の設問を支える位置づけにして、同じ問題解決を二度書かないよう棲み分ける。' },
  { case: 'case3', q: '製品特性が見込生産だった。論点はどこに寄るか？',
    a: '生産計画と在庫管理。需要予測の精度、欠品と過剰在庫が争点になる。受注生産なら生産統制と納期管理に寄る。' },
  { case: 'case3', q: '生産性向上策を並べる。前提になる取組は？',
    a: '業務プロセスの見直し。それを踏まえたうえで、省力化投資や情報技術投資、多能工化と兼任化、アウトソーシングを挙げる。' },
  { case: 'case3', q: '強みと弱みの両方を問われた。どの視点で抜き出すか？',
    a: '生産現場ではなく成長戦略の視点。市場面と製品面に分けて経営資源を挙げ、機会に強みを掛け、弱みは機会に投入して克服する。' },

  // --- 体系（事例IV）---
  { case: 'case4', q: '第1問で利益率の指標を選ぶ。どれを選ぶか？',
    a: '売上高総利益率か売上高営業利益率。経常利益率は避ける。売上原価率と販管費率の両方を出し、差の大きいほうから決める。' },
  { case: 'case4', q: '支払利息が重い会社だった。どの視点で指摘するか？',
    a: 'まず安全性で指摘できないか検証する。営業外費用の重さは利益率ではなく、負債への依存という安全性の問題として書くほうが筋が通る。' },
  { case: 'case4', q: '固定資産の効率性を指摘したい。どちらの指標を使うか？',
    a: '基本は有形固定資産回転率。投資有価証券や無形固定資産が計上されていて、そこを指摘したいときだけ固定資産回転率にする。' },
  { case: 'case4', q: '安全性の指標を選ぶ。どの順に検証するか？',
    a: '短期の安全性、次に資本構成、最後に長期の安全性。一年以内に資金が回るかをまず見て、次に負債への依存度を見る。' },
  { case: 'case4', q: '総資本利益率は二つに分解できる。何と何か？',
    a: '売上高利益率と総資本回転率。広義の収益性は、狭義の収益性と効率性の掛け算になる。どちらが悪いかで挙げる指標が決まる。' },
  { case: 'case4', q: '労働生産性はどう分解するか？',
    a: '一人あたり売上高と付加価値率の掛け算。または労働装備率と設備生産性の掛け算。分子を大きく分母を小さくする方向で助言する。' },
  { case: 'case4', q: '経営分析の記述で、指標名をそのまま書くよりよいのは？',
    a: '分母と分子の変化を書く。棚卸資産回転率が悪化した、ではなく、棚卸資産が増加したため効率性が低い、と書くほうが伝わる。' },
  { case: 'case4', q: '最終問題に「財務的視点」という制約があった。どう書くか？',
...
// 読み上げ用の英字読み。日本語音声だと綴りのまま読まれて聞き取れないため、
// カード中に実際に出てくる語だけをカタカナに置き換える。長い語から順に当てる。
const SPEECH_READINGS = [
  ['Net Operating Profit After Tax', 'ネット・オペレーティング・プロフィット・アフター・タックス'],
  ['Situational Leadership', 'シチュエーショナル・リーダーシップ'],
  ['Key Success Factor', 'キー・サクセス・ファクター'],
  ['Customer value', 'カスタマー・バリュー'],
...
function speechText(s) {
  let t = String(s);
  for (const [en, ja] of SPEECH_READINGS) t = t.split(en).join(ja);
  return t
    .replace(/（([^）]*)）/g, '、$1、')
    .replace(/\(([^)]*)\)/g, '、$1、')
    .replace(/[・／\/]/g, '、')
    .replace(/→/g, '、なので、')
    .replace(/[＝=]/g, 'は、')
    .replace(/[〜～]/g, 'から')   // 年〜月次 のような範囲表現に使われている
    .replace(/[×✕]/g, 'かける')
    .replace(/[÷]/g, 'わる')
    .replace(/[「」『』]/g, '、')   // 削ると引用句が地の文とくっついて読めなくなる
    .replace(/、{2,}/g, '、')
    // 英語とその訳が同じ語のとき「プロモーション、プロモーション」と二度読みになる
    .replace(/([ぁ-んァ-ヶー一-龥]{3,})、\1/g, '$1')
    .replace(/。、/g, '。')      // 句点の直後の読点は不自然に聞こえる
    .replace(/、。/g, '。')
    .replace(/^、|、$/g, '');
}
...
function audioDecks() {
  const decks = [];
  const common = SIGNAL_CARDS.filter(c => c.case === 'common');
  if (common.length) decks.push({
